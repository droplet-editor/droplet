...
(defun turing (lambda (tuples left right state)
  ...
  tool.onMouseDown = (event) ->
    block = a.block.findBlock (block) -> (block.paper._container.contains event.point)
    parent_root = null

    for root in roots
      if (root.paper._container.contains event.point)
        block = root
      found = root.findBlock (block) -> (block.paper._container.contains event.point)
      if found isnt root
        block = found
        parent_root = root
